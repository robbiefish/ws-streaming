...
disabled. Certificates are verified against the configured CA file, but the peer's hostname is
_not_ checked against the certificate it presents: any certificate issued by a trusted CA is
accepted, whichever host it was issued for. Finally, a `server` object holds a single TLS
